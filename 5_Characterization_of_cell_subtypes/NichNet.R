#' Run NichNET to predict the ligands specific to certain cell types based on the Differentially Expressed Genes (DEGs).
#' @param Seurat_obj_path Path of seurat object
#' @param DEG_path Number of DEG
#' @param Spec_celltype intersted cell type
#' @param ligand_target_matrix Path of ligand_target_matrix.rds(Download from the NichNET)
#' @param lr_network Path of lr_network.rds(Download from the NichNET)
#' @param weighted_network Path of weighted_network.rds(Download from the NichNET)
#' @author Ya Han

args <- commandArgs(trailingOnly = TRUE)
Seurat_obj_path <- args[1]
DEG_path <-args[2]
Spec_celltype <-args[3]
ligand_target_matrix <-args[4]
lr_network <-args[5]
weighted_network <-args[6]
...
#Need prepare object
Fibro_Seurat=readRDS(Seurat_obj_path)
Fibro_DEG=readRDS(DEG_path)
intersted_celltype=Spec_celltype
...
p_ligand_pearson = vis_ligand_pearson %>% make_heatmap_ggplot("Ligand activity", color = "darkorange",legend_position = "top", x_axis_position = "top", legend_title = "Pearson correlation coefficient\ntarget gene prediction ability)")
