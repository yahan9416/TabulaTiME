#' Evaluate the cancer type preference of cell types within each lineage
#' @param meta_info_path Path of meta.data
#' @param Datainfo_path Information of scRNA-seq datasets 
#' @param Cancer_type_info Path of TISCH_cancertype_classify.txt
#' @author Ya Han

args <- commandArgs(trailingOnly = TRUE)
meta_info_path <- args[1]
Datainfo_path <-args[2]
Cancer_type_info<-args[3]

meta.data=readRDS(meta_info_path)
...
Cancer_type_class=read.table(Cancer_type_info,header = TRUE,sep="\t")
...
pdf("CancerType_Dataset_Proportion_Cell_proportion_addWithinCancefr_Sign.pdf",height = 10,width = 8)
