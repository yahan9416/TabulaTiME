...
#' @param Column_Majorcelltype Specific cell types to genereated programs
#' @author Ya Han
...
Column_Majorcelltype<-args[2]

 Seuratobj=readRDS(Seurat_obj_path)
 
  Seuratobj@meta.data$assign.curated=Seuratobj@meta.data[match(Column_Majorcelltype,colnames(Seuratobj@meta.data))]
  ...
  batch_lineage_celltype_NMF<-function(temp_celltype){
    ...
    batch_for_patient_celltype_NMF<-function(temp_sample){
      ...
      if(length(temp_cells) >100){
        ...
        saveRDS(res,file=paste0("./",temp_dataset,"_",temp_celltype,"_",temp_sample,"_Prog10_NMF.rds"))
      }
    }
