#' Application: Benchmarking the performance of various studies on patient stratification based on estimated immune infiltration among TCGA patients.
#' @param TCGA_estimate_Infi_path Path of estimated immune infiltration for TCGA patients
#' @param TabulaTIME_ecotype_path Path of TabulaTIME defined ecotypes
#' @param Cancer_cell_path Path of result of Conserved pan-cancer microenvironment subtypes predict response to immunotherapy
#' @param Cell_ten_path Path of result of Atlas of clinically distinct cell states and ecosystems across human solid tumors
#' @param Immunity_Six_path Path of The Immune Landscape of Cancer
#' @author Ya Han

args <- commandArgs(trailingOnly = TRUE)
TCGA_estimate_Infi_path <- args[1]
TabulaTIME_ecotype_path <-args[2]
Cancer_cell_path <-args[3]
Cell_ten_path<-args[4]
Immunity_Six_path<-args[5]

#Load the estimated immune infiltration for TCGA patients
TCGA_estimate_Infi=read.table(TCGA_estimate_Infi_path,header = TRUE,sep=",")
TCGA_estimate_Infi[,1]=substring(TCGA_estimate_Infi[,1],1,12)

TabulaTIME_ecotype=readRDS(TabulaTIME_ecotype_path)
...
Cancer_cell=read.table(Cancer_cell_path,header = TRUE,fill=TRUE,sep="\t")
...
Cell_ten=read.table(Cell_ten_path,header = TRUE,fill=TRUE,sep="\t")
...
Immunity_Six=read.table(Immunity_Six_path,header = TRUE,fill=TRUE,sep="\t")
