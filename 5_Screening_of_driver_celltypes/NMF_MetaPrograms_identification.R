#NMF MPs identification

#First load the simliarity of each NMP derived programs
jaccrad_similarity=readRDS("./Pancancer_Myeloid_NMF10_program_50_features_Jaccard_similiarity_filter.rds")
...
result<-apply(matrix(1:200),1,batch_identify_MPs)
lapply(result,function(x) length(x))
result=result[1:70]
saveRDS(result,file="Pancancer_Myeloid_NMF10_program_50_features_Potentialfounder_cluster.rds")
...
save(annotation_col,jaccrad_similarity,file="Pancancer_Myeloid_NMF10_program_50_features_Potentialfounder_cluster.RData")
############MPs quality control###########

load("./Pancancer_Myeloid_NMF10_program_50_features_Potentialfounder_cluster.RData")
...
Top_Feature=readRDS("./Pancancer_MonoMacro_NMF10_program_50_features.rds")
...
gmt_path = "/fs/home/hanya/Project/Immune_pathway_enrichment/pathway/"
...
Lowquality_MPs=as.numeric(unique(c(unidataset_MPs,unicancertype_MPs,c("10","25","53"))))

result=readRDS("./Pancancer_Myeloid_NMF10_program_50_features_Potentialfounder_cluster.rds")
names(result)=1:70
...
save(MPs_cluster_result,jaccrad_similarity,annotation_result,annotation_col,file="Pancancer_Myeloid_NMF10_Top50_features_HighqualiMPs.RData")
